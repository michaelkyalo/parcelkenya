...
const COUNTIES = [
  ...
  "Garissa","Kakamega","Nyeri","Machakos","Meru","Embu","Migori","Kisii",
  "Kilifi","Lamu","Mandera","Wajir","Marsabit","Isiolo","Laikipia","Nanyuki",
  "Kericho","Bomet","Nandi","Uasin Gishu","Trans Nzoia","Bungoma","Busia",
  "Siaya","Homabay","Nyamira","Vihiga","Taita Taveta","Kwale","Tana River",
  "Samburu","Turkana","West Pokot","Elgeyo Marakwet","Baringo","Kajiado",
  "Makueni","Kitui","Tharaka Nithi","Murang'a","Kiambu","Nyandarua","Kirinyaga",
];
...
// ── Storage: uses localStorage so Admin can read on same origin ───────────────
...
.bp-page-title em { font-style: italic; color: var(--amber2); }
.bp-page-desc {
...
.bp-progress {
  ...
  margin-bottom: 52px; padding: 20px 0;
  border-top: 1px solid var(--line); border-bottom: 1px solid var(--line);
}
.bp-progress-item { display: flex; align-items: center; gap: 12px; flex: 1; }
.bp-progress-item:last-child { flex: 0; }
.bp-progress-num {
  width: 32px; height: 32px; border-radius: 50%; flex-shrink: 0;
  display: flex; align-items: center; justify-content: center;
  font-family: var(--mono); font-size: 0.65rem; font-weight: 500;
  border: 1px solid var(--line2); background: var(--ink3); color: var(--text20);
  transition: all 0.4s var(--smooth); position: relative;
}
.bp-progress-num.active {
  border-color: var(--amber); background: var(--amber-bg); color: var(--amber2);
  box-shadow: 0 0 0 4px rgba(212,168,67,0.08);
}
.bp-progress-num.done {
  border-color: var(--teal); background: var(--teal-bg); color: var(--teal); font-size: 0;
}
.bp-progress-num.done::after { content: '✓'; font-size: 0.75rem; font-family: var(--body); }
.bp-progress-label { font-size: 0.72rem; font-weight: 500; color: var(--text20); transition: color 0.3s; }
...
/* Block animation */
.bp-block { animation: blockIn 0.5s var(--smooth) both; }
@keyframes blockIn {
...
.bp-section-num {
  ...
  display: flex; align-items: center; justify-content: center;
  font-family: var(--mono); font-size: 0.6rem; color: var(--amber); flex-shrink: 0;
}
.bp-section-title {
  font-family: var(--mono); font-size: 0.6rem; letter-spacing: 2.5px;
  text-transform: uppercase; color: var(--amber); flex: 1;
}
.bp-section-line { flex: 1; height: 1px; background: linear-gradient(90deg, var(--line2), transparent); }
...
.bp-label { font-size: 0.63rem; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: var(--text20); }
...
.bp-input, .bp-select, .bp-textarea {
  ...
  font-family: var(--body); font-size: 0.86rem; color: var(--text);
...
.bp-input:focus, .bp-select:focus, .bp-textarea:focus {
  border-color: rgba(212,168,67,0.45); background: var(--ink2);
...
.bp-input-prefix {
  ...
  font-family: var(--mono); font-size: 0.82rem; color: var(--text40); pointer-events: none;
}
.bp-input.has-prefix { padding-left: 36px; }

/* Service cards */
.bp-services { display: grid; grid-template-columns: repeat(3,1fr); gap: 10px; margin-bottom: 18px; }
.bp-svc {
  border: 1px solid var(--line2); border-radius: 12px; padding: 18px 16px;
  cursor: pointer; background: var(--ink3); transition: all 0.25s var(--smooth);
  position: relative; overflow: hidden;
}
.bp-svc::after {
  content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px;
  background: linear-gradient(90deg, var(--amber), var(--amber2));
  transform: scaleX(0); transform-origin: left; transition: transform 0.3s var(--smooth);
}
...
.bp-svc-tag {
  ...
  text-transform: uppercase; color: var(--text20); background: var(--line);
...
.bp-svc-name { font-family: var(--display); font-size: 1.05rem; font-weight: 500; color: var(--text70); margin-bottom: 3px; }
.bp-svc.sel .bp-svc-name { color: var(--text); }
.bp-svc-eta { font-size: 0.72rem; color: var(--text20); font-weight: 300; margin-bottom: 12px; line-height: 1.5; }
.bp-svc-price { font-family: var(--mono); font-size: 0.78rem; color: var(--amber2); font-weight: 500; }

/* Time slots */
.bp-slots { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 18px; }
.bp-slot {
  border: 1px solid var(--line2); border-radius: 9px; padding: 12px 14px;
  cursor: pointer; background: var(--ink3); font-size: 0.8rem; color: var(--text40);
  transition: all 0.2s; display: flex; align-items: center; gap: 8px;
}
.bp-slot:hover { border-color: var(--line3); color: var(--text70); }
.bp-slot.sel { border-color: rgba(212,168,67,0.4); background: var(--amber-bg); color: var(--amber2); font-weight: 500; }
...
.bp-toggle-row {
  ...
  background: var(--ink3); cursor: pointer; transition: all 0.25s;
...
.bp-toggle-switch {
  width: 42px; height: 24px; border-radius: 12px; background: var(--line2);
  flex-shrink: 0; position: relative; transition: background 0.3s;
}
.bp-toggle-switch::after {
  content: ''; position: absolute; width: 18px; height: 18px; border-radius: 50%;
  background: var(--text20); top: 3px; left: 3px;
  transition: all 0.3s var(--spring); box-shadow: 0 1px 4px rgba(0,0,0,0.4);
}
.bp-toggle-row.on .bp-toggle-switch { background: var(--teal); }
.bp-toggle-row.on .bp-toggle-switch::after { left: 21px; background: #fff; }

/* Payment */
.bp-payments { display: grid; grid-template-columns: repeat(3,1fr); gap: 10px; margin-bottom: 20px; }
.bp-pay {
  border: 1px solid var(--line2); border-radius: 12px; padding: 18px 14px;
  cursor: pointer; background: var(--ink3); text-align: center; transition: all 0.25s var(--smooth);
}
.bp-pay:hover { border-color: var(--line3); transform: translateY(-1px); }
.bp-pay.sel { border-color: rgba(212,168,67,0.45); background: var(--amber-bg); box-shadow: 0 4px 20px rgba(0,0,0,0.3); }
...
.bp-pay-detail { background: var(--ink2); border: 1px solid var(--line); border-radius: 10px; padding: 20px 22px; margin-bottom: 20px; }
.bp-cash-alert {
...
.bp-form-footer {
  display: flex; align-items: center; justify-content: space-between;
  padding-top: 36px; margin-top: 12px; border-top: 1px solid var(--line);
}
.bp-btn-ghost {
  font-family: var(--body); font-size: 0.8rem; color: var(--text20);
  background: none; border: none; cursor: pointer;
  display: flex; align-items: center; gap: 6px; padding: 8px 0; transition: color 0.2s;
}
.bp-btn-ghost:hover { color: var(--text40); }
.bp-btn-primary {
  font-family: var(--body); font-size: 0.85rem; font-weight: 700; color: #000;
  background: linear-gradient(135deg, var(--amber) 0%, var(--amber2) 100%);
  border: none; border-radius: 9px; padding: 13px 28px; cursor: pointer;
  transition: all 0.25s var(--smooth); display: flex; align-items: center; gap: 8px;
  box-shadow: 0 4px 16px rgba(212,168,67,0.25);
}
.bp-btn-primary:hover {
  transform: translateY(-2px); box-shadow: 0 8px 28px rgba(212,168,67,0.38);
  background: linear-gradient(135deg, var(--amber2) 0%, var(--amber3) 100%);
}
.bp-btn-secondary {
  font-family: var(--body); font-size: 0.8rem; font-weight: 500; color: var(--text40);
...
.bp-panel-title { font-family: var(--display); font-size: 1.3rem; font-weight: 400; color: var(--text); letter-spacing: -0.3px; }
.bp-route { background: var(--ink3); border: 1px solid var(--line); border-radius: 12px; padding: 18px; margin-bottom: 20px; }
.bp-route-row { display: flex; align-items: center; gap: 10px; padding: 6px 0; }
.bp-route-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
...
.bp-route-line { width: 1px; height: 20px; background: var(--line2); margin-left: 3.5px; }
...
.bp-panel-svc-badge {
  font-family: var(--mono); font-size: 0.68rem; font-weight: 500; color: var(--amber2);
  background: var(--amber-bg); border: 1px solid rgba(212,168,67,0.2); border-radius: 6px; padding: 4px 10px;
}
.bp-cost { display: flex; flex-direction: column; margin-bottom: 20px; }
.bp-cost-row {
  display: flex; align-items: center; justify-content: space-between;
  padding: 11px 0; border-bottom: 1px solid var(--line); font-size: 0.79rem;
}
...
.bp-total-amount { font-family: var(--mono); font-size: 1.4rem; font-weight: 500; color: var(--amber2); letter-spacing: -0.5px; }
.bp-checklist { display: flex; flex-direction: column; gap: 8px; margin-top: auto; padding-top: 28px; border-top: 1px solid var(--line); }
.bp-check-item { display: flex; align-items: center; gap: 10px; font-size: 0.75rem; color: var(--text20); }
...
.bp-check-item.done .bp-check-dot { background: var(--teal-bg); border-color: var(--teal); color: var(--teal); }
...
.bp-confirm-aura { position: relative; width: 110px; height: 110px; margin-bottom: 32px; }
...
@keyframes ringPop { from { opacity: 0; transform: scale(0.5); } to { opacity: 1; transform: scale(1); } }
.bp-confirm-status {
  font-family: var(--mono); font-size: 0.6rem; letter-spacing: 3px;
...
.bp-confirm-sub { font-size: 0.82rem; color: var(--text40); margin-bottom: 36px; font-weight: 300; }
.bp-tracking-id {
...
.bp-confirm-table {
  width: 100%; max-width: 520px; background: var(--ink3); border: 1px solid var(--line);
  border-radius: 14px; overflow: hidden; margin-bottom: 32px;
}
.bp-confirm-row {
  display: flex; justify-content: space-between; align-items: center;
  padding: 13px 20px; border-bottom: 1px solid var(--line); font-size: 0.8rem;
}
      ...
      {steps.map((label, i) => {
        ...
        return (
          <div className="bp-progress-item" key={label}>
            <div className={`bp-progress-num ${isActive ? "active" : isDone ? "done" : ""}`}>{!isDone && n}</div>
...
function SummaryPanel({ form, fee, weightCost, total }) {
  ...
  return (
    <aside className="bp-panel">
      ...
      <div className="bp-route">
        <div className="bp-route-row">
          ...
          <div>
            ...
            <div className={`bp-route-value ${form.senderCounty ? "" : "placeholder"}`}>{form.senderCounty || "Select county"}</div>
          </div>
        </div>
        <div style={{ display: "flex", paddingLeft: "3.5px" }}><div className="bp-route-line" /></div>
        <div className="bp-route-row">
          <div className="bp-route-dot dest" />
          <div>
            <div className="bp-route-label">DESTINATION</div>
            <div className={`bp-route-value ${form.recipientCounty ? "" : "placeholder"}`}>{form.recipientCounty || "Select county"}</div>
          </div>
        </div>
      </div>
      ...
      <div className="bp-cost">
        <div className="bp-cost-row"><span>Base delivery fee</span><strong>KSh {fee.toLocaleString()}</strong></div>
        <div className="bp-cost-row"><span>Weight ({form.weight || "0"} kg)</span><strong>KSh {weightCost.toLocaleString()}</strong></div>
        {form.insurance && <div className="bp-cost-row"><span>Insurance cover</span><strong>KSh 80</strong></div>}
      </div>
  ...
  return (
    <div className="bp-block">
      ...
      <div className="bp-grid-2">
        <Field label="Full Name"><input className="bp-input" placeholder="John Kamau" value={form.senderName} onChange={e => u("senderName", e.target.value)} /></Field>
      ...
      <div className="bp-grid-2">
        <Field label="Email (optional)"><input className="bp-input" placeholder="john@example.com" value={form.senderEmail} onChange={e => u("senderEmail", e.target.value)} /></Field>
      ...
      <Field label="Pickup Address"><input className="bp-input" placeholder="House No., Street, Estate, Landmark" value={form.senderAddress} onChange={e => u("senderAddress", e.target.value)} /></Field>

      <Section num="B" title="Recipient Information" />
      <div className="bp-grid-2">
        <Field label="Full Name"><input className="bp-input" placeholder="Jane Wanjiku" value={form.recipientName} onChange={e => u("recipientName", e.target.value)} /></Field>
        <Field label="Phone Number">
          <div className="bp-input-wrap">
            <span className="bp-input-prefix">🇰🇪</span>
      ...
      <div className="bp-grid-2">
        ...
        <Field label="Delivery Address"><input className="bp-input" placeholder="House No., Street, Estate, Landmark" value={form.recipientAddress} onChange={e => u("recipientAddress", e.target.value)} /></Field>
      </div>

      <Section num="C" title="Parcel Details" />
      <div className="bp-grid-2">
        <Field label="Weight (kg)"><input className="bp-input" type="number" placeholder="0.5" min="0.1" max="50" step="0.1" value={form.weight} onChange={e => u("weight", e.target.value)} /></Field>
      ...
      <div className="bp-form-footer">
        <div />
        <button className="bp-btn-primary" onClick={onNext}>Schedule Pickup <span>→</span></button>
      </div>
    </div>
  );
}
...
function Step2({ form, u, onBack, onNext }) {
  ...
  return (
    <div className="bp-block">
      ...
      <Field label="Pickup Date"><input className="bp-input" type="date" value={form.date} onChange={e => u("date", e.target.value)} style={{ maxWidth: 240 }} /></Field>
  ...
  return (
    <div className="bp-block">
      ...
      <div className="bp-payments">
        {[{ val: "mpesa", icon: "📱", name: "M-Pesa" }, { val: "card", icon: "💳", name: "Card" }, { val: "cash", icon: "💵", name: "Cash" }].map(p => (
      ...
      {form.payment === "mpesa" && (
        <div className="bp-pay-detail">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--ink2)", border: "1px solid var(--line2)", borderRadius: 9, padding: "0 14px", height: 46 }}>
            <span>🇰🇪</span>
            <input style={{ flex: 1, background: "none", border: "none", outline: "none", fontFamily: "var(--mono)", fontSize: "0.88rem", color: "var(--text)" }} placeholder="+254 700 000 000" value={form.mpesaPhone} onChange={e => u("mpesaPhone", e.target.value)} />
          </div>
          <div style={{ fontSize: "0.7rem", color: "var(--text20)", marginTop: 8 }}>An M-Pesa STK push will be sent to authorise payment.</div>
        </div>
      )}
      {form.payment === "card" && (
        <div className="bp-pay-detail">
          <Field label="Card Number"><input className="bp-input" placeholder="4111 1111 1111 1111" /></Field>
          <div className="bp-grid-2">
      ...
      {form.payment === "cash" && (
        <div className="bp-cash-alert">💡 Pay your rider in cash at the pickup address. Please prepare <strong>KSh {total.toLocaleString()}</strong> in exact change.</div>
      )}
      <div className="bp-form-footer">
        <button className="bp-btn-ghost" onClick={onBack}>← Back</button>
...
function Step4({ form, total, trackingId, onNew }) {
  return (
    <div className="bp-confirm">
      <div className="bp-confirm-aura"><div className="bp-confirm-ring">✓</div></div>
      ...
      <p className="bp-confirm-note">
        ...
        Save your tracking ID to monitor delivery progress.
      </p>
...
export default function BookPage({ setPage }) {
  ...
  // ── Saves to localStorage — no async needed ──────────────────────────────
  const handleConfirm = () => {
    const booking = {
  ...
  return (
    <div className="bp-root">
      ...
      <nav className="bp-nav">
        <div className="bp-nav-brand" onClick={() => setPage && setPage("home")}>
          <div className="bp-nav-brand-dot" />SpeedPak
        </div>
        <div className="bp-nav-center">
          New Shipment <span>/</span> {stepTitles[step]}
        </div>
        <div className="bp-nav-actions">
          <button className="bp-nav-btn" onClick={() => setPage && setPage("home")}>← Home</button>
        </div>
      </nav>
      ...
      <div className="bp-layout">
        ...
        {step < 4 && <SummaryPanel form={form} fee={fee} weightCost={weightCost} total={total} />}
      </div>
    </div>
  );
}